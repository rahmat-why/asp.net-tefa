            ...
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                using (PdfWriter writer = new PdfWriter(stream))
                {
                    ...
                    using (PdfDocument pdf = new PdfDocument(writer))
                    {
                        ...
                        if (eksportpdf.RepairStatus != "SELESAI")
                        {
                            TempData["ErrorMessage"] = "Invoice belum bisa diunduh";
                            return RedirectToAction("History", "Booking");
                        }
                        ...
                        Text text2 = new Text("Politeknik Astra").SetFontSize(10f);
                        ...
                        document.Add(new Paragraph($"INVOICE # {eksportpdf.IdBooking}").SetTextAlignment(iText.Layout.Properties.TextAlignment.CENTER));
                        ...
                        table.AddCell(new Cell().Add(new Paragraph($"No.Telpon : {eksportpdf.IdVehicleNavigation.IdCustomerNavigation.Phone }")).SetBorder(Border.NO_BORDER));
                        ...
                        if (eksportpdf.AdditionalReplacementPart == null)
                        {
                            ...
                            table2.AddCell(new Cell().Add(new Paragraph($"Total : Rp. {eksportpdf.Price:N2}")).SetBorder(Border.NO_BORDER));
                        }
                        else
                        {
                            table2.AddCell(new Cell().Add(new Paragraph("Perbaikan").SetTextAlignment(iText.Layout.Properties.TextAlignment.CENTER)));
                            ...
                            table2.AddCell(new Cell().Add(new Paragraph($"Total : Rp. {eksportpdf.Price + eksportpdf.AdditionalPrice:N2}")).SetBorder(Border.NO_BORDER));
                        }
                        document.Add(table2);
